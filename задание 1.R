...
#Построить карту на которой будет по районам отображена средняя высота доминирующего вида растений средняя высота липы европейской по районам
...
# Фильтруем по виду
average_heights = greendb %>% filter(species_ru == "Липа европейская", adm_region != "NA") %>% group_by(adm_region )%>%
  summarise(
